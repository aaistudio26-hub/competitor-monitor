...
class HistoryService:
    ...
    def __init__(self):
        ...
        logger.info("History сервис инициализирован OK")
        logger.info("=" * 50)
    ...
    def add_entry(
        ...
        if old_count > len(history):
            logger.info(f"  Удалено старых записей: {old_count - len(history)}")
    ...
    def get_history(self) -> List[HistoryItem]:
        ...
        for item in history:
            try:
                ...
                logger.warning(f"  Пропуск битой записи истории: {e}")
        return items
